...
ParserResult<Stmt> Parser::parseStmtDefer() {
  ...
  auto *const tempDecl = FuncDecl::createImplicit(
      Context, StaticKind::None, name, /*NameLoc=*/PreviousLoc,
      /*Async=*/false,
      /*Throws=*/false,
...
static void parseGuardedPattern(Parser &P, GuardedPattern &result,
  ...
  if (parsingContext == GuardedPatternContext::Catch &&
    ...
    auto *var = VarDecl::createImplicit(P.Context, StaticKind::None,
...

  {
    ...
    for (unsigned i : indices(tmp)) {
      ...
      Result[i] = VarDecl::createImplicit(
          Context, StaticKind::None, vOld->getIntroducer(), vOld->getNameLoc(),
...
parseStmtCase(Parser &P, SourceLoc &CaseLoc,
...

  {
    ...
    for (unsigned i : indices(tmp)) {
      auto *vOld = tmp[i];
      Result[i] = VarDecl::createImplicit(
          P.Context, StaticKind::None, vOld->getIntroducer(),
