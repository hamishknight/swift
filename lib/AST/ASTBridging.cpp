...
DiagnosticEngine *getDiagnosticEngine(BridgedDiagnosticEngine bridged) {
  return static_cast<DiagnosticEngine *>(bridged.raw);
}
...
bool BridgedNominalTypeDecl_isStructWithUnreferenceableStorage(BridgedNominalTypeDecl decl) {
  if (auto *structDecl = dyn_cast<swift::StructDecl>(decl.ptr))
...
#define AST_BRIDGING_WRAPPER_NULLABLE(Name)                                    \
  [[maybe_unused]]                                                             \
  static Name *unbridged(BridgedNullable##Name bridged) {                      \
    return bridged.ptr;                                                        \
  }
...
static inline SourceRange unbridged(BridgedSourceRange cRange) {
  return SourceRange(cRange.startLoc, cRange.endLoc);
}
...
BridgedParamDecl ParamDecl_createParsed(
  ...
  assert(cSecondNameLoc.isValid() == (bool)cSecondName.raw);
  if (!cSecondName.raw) {
...
BridgedConstructorDecl ConstructorDecl_createParsed(
    ...
    BridgedSourceLoc cThrowsLoc, BridgedNullableTypeRepr thrownType,
    BridgedNullableTrailingWhereClause genericWhereClause) {
  assert(cFailabilityMarkLoc.isValid() || !isIUO);
...
BridgedOperatorDecl OperatorDecl_createParsed(
  ...
  assert(cColonLoc.isValid() == (bool)cPrecedenceGroupName.raw);
  assert(cColonLoc.isValid() == cPrecedenceGroupLoc.isValid());
