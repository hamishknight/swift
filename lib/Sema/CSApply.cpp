  ...
  private:
    ...
    Expr *buildSubscriptHelper(Expr *base, ArgumentList *args,
      ...
      if (choice.getKind() == OverloadChoiceKind::KeyPathApplication) {
        ...
        auto *indexKP = args->getUnaryArgExpr();
        assert(indexKP);
...
#include "swift/AST/MagicIdentifierKinds.def"
  ...
  public:
    ...
    Expr *visitEditorPlaceholderExpr(EditorPlaceholderExpr *E) {
      ...
      auto *argList = ArgumentList::forImplicitUnlabelled(ctx, {argExpr});
    ...
    Expr *visitKeyPathExpr(KeyPathExpr *E) {
      ...
      auto *argList = ArgumentList::forImplicitUnlabelled(ctx, {E});
...
ArgumentList *ExprRewriter::coerceCallArguments(
  ...
  for (unsigned paramIdx = 0, numParams = parameterBindings.size();
    ...
    if (args->getFirstTrailingClosureExpr() == argExpr)
    ...
    if (paramInfo.hasExternalPropertyWrapper(paramIdx)) {
      ...
      argExpr = AppliedPropertyWrapperExpr::create(
          ctx, callee, param, argExpr->getStartLoc(), wrapperType, argExpr,
          valueKind);
    ...
    if (argRequiresAutoClosureExpr(param, argType)) {
      ...
      if (shouldInjectWrappedValuePlaceholder) {
        ...
        auto *placeholder = injectWrappedValuePlaceholder(
            cs.buildAutoClosureExpr(argExpr, closureType,
      ...
      convertedArg = cs.buildAutoClosureExpr(argExpr, closureType);
    } else {
  ...
  return ArgumentList::create(ctx, args->getLParenLoc(), newArgs,
                              args->getRParenLoc(), newTrailingClosureIdx,
                              args->isImplicit());
}
  ...
  auto finishApplyOfDeclWithSpecialTypeCheckingSemantics
    = [&](ApplyExpr *apply,
      ...
      case DeclTypeCheckingSemantics::WithoutActuallyEscaping: {
        ...
        auto *argList = ArgumentList::forImplicitUnlabelled(ctx, {escapable});
      ...
      case DeclTypeCheckingSemantics::OpenExistential: {
        ...
        auto *argList = ArgumentList::forImplicitUnlabelled(ctx, {opaqueValue});
        auto callSubExpr = CallExpr::createImplicit(ctx, body, argList);
        cs.cacheSubExprTypes(callSubExpr);
                                                               ...
                                                               Expr *rootExpr)
{
  ...
  if (auto *BE = dyn_cast<BinaryExpr>(parent)) {
    if (BE->getLHS() == expr)
      return {parent, 0};
    if (BE->getRHS() == expr)
      return {parent, 1};
