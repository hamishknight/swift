...
applySolutionToPattern(Pattern *pattern, Solution &solution,
                       SyntacticElementTarget target,
                       RewriteTargetFn rewriteTarget) {
...
applySolutionToInitialization(Solution &solution, SyntacticElementTarget target,
  ...
  if (auto pattern = applySolutionToPattern(target.getInitializationPattern(),
                                            solution, target, rewriteTarget)) {
    resultTarget.setPattern(pattern.get());
  } else {
    return std::nullopt;
  }
...
static std::optional<SyntacticElementTarget> applySolutionToForEachStmt(
  ...
  // Apply the solution to the pattern as well.
  auto pattern = applySolutionToPattern(target.getForEachPattern(), solution,
                                        target, rewriteTarget);
  if (!pattern)
    return std::nullopt;

  stmt->setPattern(pattern.get());
  resultTarget.setPattern(pattern.get());
...
ExprWalker::rewriteTarget(SyntacticElementTarget target) {
  ...
  if (auto expr = target.getAsExpr()) {
    ...
    auto pattern = applySolutionToPattern(info.pattern, solution, target,
                                          rewriteTargetFn);
    if (!pattern)
      return std::nullopt;
    ...
    if (auto resultPattern = applySolutionToPattern(pattern, solution, target,
                                                    rewriteTargetFn)) {
      result.setPattern(resultPattern.get());
      return result;
    }
