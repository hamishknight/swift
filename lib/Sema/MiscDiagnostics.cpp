  ...
  PreWalkResult<Expr *> walkToExprPre(Expr *E) override {
    if (auto *SVE = dyn_cast<SingleValueStmtExpr>(E)) {
      ...
      case IsSingleValueStmtResult::Kind::Valid:
        break;
      case IsSingleValueStmtResult::Kind::UnterminatedBranch: {
        auto *branch = mayProduceSingleValue.getUnterminatedBranch();
        Diags.diagnose(branch->getEndLoc(),
                       diag::single_value_stmt_branch_must_return_or_throw,
                       S->getKind());
        break;
      }
      ...
      case IsSingleValueStmtResult::Kind::InvalidJumps: {
        ...
        for (auto *JumpStmt : mayProduceSingleValue.getInvalidJumps()) {
          Diags.diagnose(JumpStmt->getStartLoc(),
                         diag::cannot_jump_in_single_value_stmt,
                         JumpStmt->getKind(), S->getKind());
        }
        break;
      }
