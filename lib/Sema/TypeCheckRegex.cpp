...
RegexLiteralPatternInfoRequest::evaluate(Evaluator &eval,
  ...
  BridgedRegexPatternFeatures bridgedFeatures;
  SWIFT_DEFER {
    swift_ASTGen_freeBridgedRegexPatternFeatures(bridgedFeatures);
  };
  ...
  SmallVector<RegexLiteralExpr::PatternFeature> features;
...
StringRef RegexLiteralFeatureDescriptionRequest::evaluate(
    Evaluator &evaluator, RegexLiteralPatternFeatureKindKey key) const {
#if SWIFT_BUILD_REGEX_PARSER_IN_COMPILER
  // The resulting string is allocated in the ASTContext, we can return the
  // StringRef directly.
  BridgedStringRef str;
  swift_ASTGen_getDescriptionForRegexPatternFeature(key.feature, *key.context,
                                                    &str);
...
AvailabilityRange RegexLiteralFeatureAvailabilityRequest::evaluate(
  ...
  swift_ASTGen_getSwiftVersionForRegexPatternFeature(key.feature, &version);
  return key.context->getSwiftAvailability(version.getMajor(),
                                           version.getMinor());
#else
  llvm_unreachable("Shouldn't have parsed a RegexLiteralExpr");
#endif
