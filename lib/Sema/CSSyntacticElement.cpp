...
bool ConstraintSystem::generateConstraints(SingleValueStmtExpr *E) {
  ...
  if (branchTypes.empty()) {
    ...
    // expressions that have branches that all end in a return or throw, and
    // we'll warn that we've inferred Void.
...
void ConjunctionElement::findReferencedVariables(
  ...
  for (auto *tv : referencedVars) {
    if (!tv->getImpl().getLocator()->isLastElement<LocatorPathElt::SingleValueStmtBranch>())
      continue;
