...
public:
  ...
  Stmt *visitThenStmt(ThenStmt *TS) {
    // These get type-checked in the constraint system, and we emit an
    // out-of-place diagnostic for any that aren't in MiscDiagnostics. Let's
    // type-check the expression to get any extra diagnostics though.
    auto *E = TS->getResult();
