...
public:
  ...
  void visitDeclRefExpr(DeclRefExpr *e) {
    ...
    auto *unaryArg = thisCallSite->getArgs()->getUnaryArgExpr();
...
CallEmission CallEmission::forApplyExpr(SILGenFunction &SGF, ApplyExpr *e) {
  ...
  if (auto *call = apply.callSite) {
    ...
    if (call->implicitlyAsync()) {
      ...
      if (auto globalActorTy = fnTy->getGlobalActor())
        isolation = ActorIsolation::forGlobalActor(globalActorTy, false);
...
SILGenFunction::prepareSubscriptIndices(SubscriptDecl *subscript,
  ...
  for (auto substParam : substParams) {
    ...
    result.add(argList->getExpr(argValues.size() - count), std::move(elt));
...
RValue SILGenFunction::emitDynamicSubscriptExpr(DynamicSubscriptExpr *e,
  ...
  // Emit the index.
  auto *indexExpr = e->getArgs()->getUnaryArgExpr();
  assert(indexExpr);
