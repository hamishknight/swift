...
// CHECK-LABEL: sil_coverage_map {{.*}} "$s16coverage_if_expr0b1_C12_with_returnSiyF"
func if_expr_with_return() -> Int { // CHECK:      [[@LINE]]:35 -> {{[0-9]+}}:2 : 0
  if .random() {                    // CHECK-NEXT: [[@LINE]]:6 -> [[@LINE]]:15 : 0
    return 0                        // CHECK-NEXT: [[@LINE-1]]:16 -> [[@LINE+2]]:4 : 1
                                    // CHECK-NEXT: [[@LINE+1]]:4 -> {{[0-9]+}}:2 : (0 - 1)
  } else if .random() {             // CHECK-NEXT: [[@LINE]]:13 -> [[@LINE]]:22 : (0 - 1)
    1                               // CHECK-NEXT: [[@LINE-1]]:23 -> [[@LINE+1]]:4 : 2
  } else {                          // CHECK-NEXT: [[@LINE]]:4 -> {{[0-9]+}}:2 : (0 - 1)
    2                               // CHECK-NEXT: [[@LINE-1]]:10 -> [[@LINE+4]]:4 : ((0 - 1) - 2)

                                    // FIXME: Incorrect
                                    // CHECK-NEXT: [[@LINE+1]]:4 -> [[@LINE+2]]:2 : (0 - 1)
  }                                 // CHECK-NEXT: }
}
